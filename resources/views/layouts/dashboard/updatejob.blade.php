        ...
        <div class="page container mx-auto px-2 sm:px-0 bg-white px-2 sm:px-4 py-4">
            <div class="flex flex-wrap">
                <div class="w-full sm:w-8/12">
                    ...
                    <div x-data="{message: true}">

                        <h4 class="text-gray-600 text-center font-bold">Editing Current Job Post</h4>
                        @if ($errors->any())
                        ...
                        @if (session('postStatus'))
                                <div class="bg-teal-100 shadow border-teal-400 border-t-2 pl-2 pr-1 py-3 sm:w-4/5 my-2 mx-auto flex justify-evenly" role="alert">
                                    ...
                                    <span class="block sm:inline text-sm w-4/5 font-semibold text-gray-700">{{session('postStatus')}}</span>
                                    <span class="text-right" role="button" @click="$event.target.closest('div').style.display = 'none'">
                                        <svg class="h-6 w-6 text-gray-700 fill-current" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><title>Close</title>
                                            <path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z"/>
                                        </svg>
                        ...
                        @endif
                          ...
                          <div class="flex justify-center">
                              {{-- @dump($job) --}}
                                 <form method="POST" action="{{route('update:jobpost')}}" class="border p-2 mt-4 w-full sm:w-4/5" x-data="utils()" enctype="multipart/form-data" id="postjobform">
                                    <div class="my-4 mx-2">
                                        <label class="text-gray-700 text-sm bold" for="job_title">Job title*</label>
                                        <input type="text" placeholder="enter job title" id="job_title" value="{{old('job_title', $job->job_title)}}" required name="job_title" class="input">
                                    </div>
                                        ...
                                        <div class="my-4 mx-2 sm:w-2/5">
                                            ...
                                            <span class="flex flex-wrap justify-between text-lg text-gray-600">
                                                <span class="inner-form-wrapper mx-2">
                                                    <input type="radio" name="job_type" {{($job->job_type == 'full time') ? 'checked' : ''}} value="full time" id="full_time">
                                                    <label for="full_time">Full time</label>
                                                </span>

                                                <span class="inner-form-wrapper mx-2">
                                                    <input type="radio" name="job_type" {{($job->job_type == 'internship') ? 'checked': ''}} value="internship" id="internship">
                                                    <label for="internship">Internship</label>
                                                </span>

                                                <span class="inner-form-wrapper mx-2">
                                                    <input type="radio" name="job_type" {{($job->job_type == 'part time') ? 'checked': ''}}  value="part time" id="part_time">
                                                    <label for="part_time">Part time</label>
                                                </span>
                                                <span class="inner-form-wrapper mx-5 sm:mx-2 md:mx-0">
                                                    <input type="radio" name="job_type" {{($job->job_type == 'contract') ? 'checked': ''}} value="contract" id="contract">
                                        ...
                                        <div class="my-4 mx-2  sm:w-2/5">
                                            ...
                                            <select class="input select" name="job_qualification" id="job_qualification">
                                                @if (old('job_qualification') )
                                                   ...
                                                   @else
                                                   <option value="{{$job->job_qualification}}">{{$job->job_qualification}}</option>
                                                @endif                                                
                                        ...
                                        <div class="my-4 mx-2  sm:w-1/2">
                                            ...
                                            <select class="input select" name="job_experience" id="job_experience">
                                                @if ( old('job_experience') )
                                                   ...
                                                   @else
                                                   <option value="{{$job->job_experience}}">{{$job->job_experience}}</option>
                                                @endif
                                                <option value="Any level">Entry level</option>
                                    ...
                                    <div class="block sm:flex justify-start items-center" x-data="{toggle: '0'}">
                                        ...
                                        <div class="my-4 mx-2  sm:w-1/2" x-show="toggle==1">
                                               ...
                                               <input type="text" class="input" name="expected_salary" value="{{old('expected_salary', $job->expected_salary)}}" placeholder="GHS1000 or GHS1500 - GHS2000">
                                        </div>
                                     </div>
                                    ...
                                    <div class="block sm:flex  flex-wrap justify-between">
                                        ...
                                        <div class="my-4 mx-2  sm:w-1/2">
                                            ...
                                            <input type="text" placeholder="example.com/jobs/accountants-recruitment/3211." name="application_url" value="{{old('application_url', $job->application_url)}}"  id="application_url" class="input">
                                        </div>
                                    </div>
                                    ...
                                    <div class="my-4 mx-2">
                                        ...
                                        <textarea placeholder="Full job description" rows="8" class="input text-left" name="job_description" id="job_description">{{old('job_description', $job->job_description)}}</textarea>
                                    </div>
                                    <div>
                                        @csrf
                                        @method('POST')
                                        <input type="hidden" name="_job" value="{{ base64_encode($job->id) }}">
                                    </div>

                                <div class="my-4 mx-2  sm:w-1/2" x-data="{onsite:'{{$job->onsite}}', modal:false}">
                                                ...
                                                <input type="checkbox" {{($job->onsite) ? 'checked' : ''}} {{(old($job->onsite)) ? 'checked' : ''}} id="onsite" name="onsite" 
                                                    class="appearance-none w-full h-full active:outline-none focus:outline-none"
                                    ...
                                    <div class="my-4 mx-2 flex flex-wrap justify-center items-center">
                                        <label for="company_logo" class="text-gray-300 text-sm bold block border rounded p-2 h-32 mx-auto text-center w-56 cursor-pointer">
                                               ...
                                               @if ($job->company_logo)
                                                   <img src="{{asset('storage/uploads/'.$job->company_logo)}}" class="img-fluid mx-auto" width="100">
                                               @else
